...
class SolicitudModel:
    ...
    def crear_solicitud(material_id, cantidad_solicitada, usuario_solicitante, oficina_solicitante, porcentaje_oficina=100, observacion=""):
        """
        MÃ©todo wrapper para crear una solicitud con la firma esperada por el blueprint.
        Llama internamente al mÃ©todo crear() con los parÃ¡metros correctos.
        """
        ...
        # Llamar al mÃ©todo crear original con los parÃ¡metros correctos
    ...
    def aprobar(solicitud_id, usuario_aprobador_id):
        conn = get_database_connection()
        if conn is None:
            return False, "Ã¢ÂÅ’ Error de conexiÃƒÂ³n a la base de datos"
        cursor = conn.cursor()
        try:
            # PRIMERO: Obtener informaciÃƒÂ³n de la solicitud
            cursor.execute("""
            ...
            if not solicitud_info:
                return False, "Ã¢ÂÅ’ Solicitud no encontrada o no estÃƒÂ¡ pendiente"
            
            material_id, cantidad_solicitada, estado_id, valor_unitario, stock_disponible, porcentaje_oficina = solicitud_info
            
            # VERIFICAR STOCK DISPONIBLE
            if cantidad_solicitada > stock_disponible:
                return False, f"Ã¢ÂÅ’ Stock insuficiente. Disponible: {stock_disponible}, Solicitado: {cantidad_solicitada}"
            ...
            # EJECUTAR APROBACIÃƒâ€œN COMPLETA
            cursor.execute("""
                BEGIN TRANSACTION;
                ...
                ) VALUES (?, ?, 'Sistema', 'AprobaciÃƒÂ³n completa');
            ...
            conn.commit()
            return True, f"Ã¢Å“â€¦ Solicitud aprobada exitosamente. Stock actualizado: -{cantidad_solicitada} unidades"
            
        except Exception as e:
            conn.rollback()
            err = str(e)
            if "LÃƒÂ­mite mensual" in err:
                return False, "Ã¢ÂÅ’ LÃƒÂ­mite mensual excedido"
            if "Stock insuficiente" in err or "excede el inventario" in err:
                return False, "Ã¢ÂÅ’ Stock insuficiente"
            if "Solicitud no encontrada" in err:
                return False, "Ã¢ÂÅ’ Solicitud no encontrada"
            return False, f"Ã¢ÂÅ’ Error al aprobar: {err}"
        finally:
            cursor.close()
            conn.close()
    ...
    def aprobar_parcial(solicitud_id, usuario_aprobador_id, cantidad_aprobada):
        conn = get_database_connection()
        if conn is None:
            return False, "Error de conexiÃƒÂ³n"
        cursor = conn.cursor()
        try:
            aprobador_id = SolicitudModel._obtener_aprobador_id(usuario_aprobador_id)
            cursor.execute(
            ...
            conn.commit()
            return True, f"Ã¢Å“â€¦ {cantidad_aprobada} unidades aprobadas y entregadas"
        except Exception as e:
            conn.rollback()
            err = str(e)
            if "Cantidad aprobada invÃƒÂ¡lida" in err:
                return False, "Ã¢ÂÅ’ Cantidad aprobada invÃƒÂ¡lida"
            if "solicitudes pendientes" in err:
                return False, "Ã¢ÂÅ’ Solo solicitudes pendientes"
            if "Solicitud no encontrada" in err:
                return False, "Ã¢ÂÅ’ Solicitud no encontrada"
            return False, f"Ã¢ÂÅ’ Error al aprobar parcialmente: {err}"
        finally:
            cursor.close()
            conn.close()
    ...
    def rechazar(solicitud_id, usuario_aprobador_id, observacion=""):
        ...
        try:
            ...
            conn.commit()
            return True, "âœ“ Solicitud rechazada exitosamente"
        except Exception as e:
            conn.rollback()
            return False, f"âŒ Error al rechazar: {str(e)}"
        finally:
            cursor.close()
            conn.close()
    ...
    def obtener_info_devolucion(solicitud_id):
        ...
        try:
            ...
            print("Ã¢ÂÅ’ ERROR en obtener_info_devolucion:", str(e))
            return None
        finally:
            cursor.close()
            conn.close()
    ...
    def registrar_devolucion(solicitud_id, cantidad_devuelta, usuario_nombre, observacion=""):
        conn = get_database_connection()
        if conn is None:
            return False, "Ã¢ÂÅ’ Error de conexiÃƒÂ³n"
        cursor = conn.cursor()
        try:
            cursor.execute("""
                SELECT 
            ...
            if estado_id not in (2, 4):
                return False, "Ã¢ÂÅ’ Solo se pueden devolver solicitudes aprobadas o entregadas"

            if cantidad_devuelta <= 0:
                return False, "Ã¢ÂÅ’ La cantidad a devolver debe ser mayor a 0"

            if cantidad_devuelta > cantidad_puede_devolver:
                return False, f"Ã¢ÂÅ’ No puede devolver mÃƒÂ¡s de {cantidad_puede_devolver} unidades"
            ...
            conn.commit()
            return True, "Ã¢Å“â€¦ DevoluciÃƒÂ³n registrada exitosamente"
        except Exception as e:
            conn.rollback()
            return False, f"Ã¢ÂÅ’ Error en devoluciÃƒÂ³n: {str(e)}"
        finally:
            cursor.close()
            conn.close()
    ...
    def obtener_todas(estado=None, oficina=None, material=None, solicitante=None):
        """
        Obtiene todas las solicitudes con informaciÃƒÂ³n de novedades
        """
        conn = get_database_connection()
        if conn is None:
            return []
        ...
        try:
            sql = """
                SELECT 
                    ...
                    -- Obtener informaciÃƒÂ³n de novedad si existe
                    ns.EstadoNovedad,
                    ns.TipoNovedad,
                    ...
                    -- Ã¢Å“â€¦ Agregar imagen del material y de la novedad
            ...
            if estado and estado != 'todos':
                ...
                # Si el estado es un nombre (string no numÃƒÂ©rico), buscar por nombre de estado
                elif not str(estado).isdigit():
                    where_clauses.append("es.NombreEstado = ?")
                    params.append(estado)
                # Si es un nÃƒÂºmero, buscar por ID
                else:
            ...
            print(f"Ã¢ÂÅ’ Error obteniendo solicitudes: {e}")
            import traceback
            traceback.print_exc()
            return []
        finally:
    ...
    def obtener_por_id(solicitud_id):
        ...
        try:
            ...
            if rows:
                ...
                # Ã¢Å“â€¦ Agregar imagen del material
    ...
    def _obtener_aprobador_id(usuario_id):
        ...
        try:
            ...
            print(f"Ã¢Å¡Â Ã¯Â¸Â Usuario {usuario_id} no encontrado. Usando aprobador por defecto.")
            cursor.execute("SELECT TOP 1 AprobadorId FROM Aprobadores WHERE Activo = 1 ORDER BY AprobadorId")
            aprobador_default = cursor.fetchone()
            return aprobador_default[0] if aprobador_default else 1
        
        except Exception as e:
            print(f"Ã¢ÂÅ’ Error obteniendo aprobador_id: {e}")
            import traceback
            traceback.print_exc()
    ...
    # ==========================
    # MÃƒâ€°TODOS ADICIONALES
    # ==========================

    @staticmethod
    def obtener_estadisticas_por_material(material_id):
        """Obtiene estadÃƒÂ­sticas de solicitudes para un material especÃƒÂ­fico"""
        conn = get_database_connection()
        if conn is None:
        ...
        try:
            ...
            print(f"Error obteniendo estadÃƒÂ­sticas para material {material_id}: {e}")
            return [0, 0, 0, 0, 0, 0, 0]
        finally:
            cursor.close()
            conn.close()
    ...
    # ==========================
    # DEVOLUCIONES CON APROBACIÃƒâ€œN
    # ==========================

    @staticmethod
    def solicitar_devolucion(solicitud_id, cantidad_devuelta, usuario_solicita, motivo="", ruta_imagen=None):
        """
        Registra una SOLICITUD de devoluciÃƒÂ³n (pendiente de aprobaciÃƒÂ³n).
        ...
        try:
            ...
            # Verificar que no haya otra devoluciÃƒÂ³n pendiente
            cursor.execute("""
            ...
            if pendientes > 0:
                return False, "Ã¢ÂÅ’ Ya existe una solicitud de devoluciÃƒÂ³n pendiente para esta solicitud"
            
            # Insertar solicitud de devoluciÃƒÂ³n con estado PENDIENTE
            cursor.execute("""
                INSERT INTO Devoluciones (
                    SolicitudId, MaterialId, CantidadDevuelta, FechaDevolucion,
            ...
            conn.commit()
            return True, "Ã¢Å“â€¦ Solicitud de devoluciÃƒÂ³n registrada. Pendiente de aprobaciÃƒÂ³n."
            
        except Exception as e:
            conn.rollback()
            print(f"Ã¢ÂÅ’ Error en solicitud de devoluciÃƒÂ³n: {e}")
            import traceback
            traceback.print_exc()
    ...
    def aprobar_devolucion(devolucion_id, usuario_aprueba, observaciones=""):
        """
        Aprueba una devoluciÃƒÂ³n y actualiza el stock.
        """
        conn = get_database_connection()
        if conn is None:
            return False, "Ã¢ÂÅ’ Error de conexiÃƒÂ³n a la base de datos"
        ...
        cursor = conn.cursor()
        try:
            # Obtener informaciÃƒÂ³n de la devoluciÃƒÂ³n
            cursor.execute("""
            ...
            if not row:
                return False, "Ã¢ÂÅ’ DevoluciÃƒÂ³n no encontrada"
            
            solicitud_id, material_id, cantidad_devuelta, estado = row
            
            if estado != 'PENDIENTE':
                return False, f"Ã¢ÂÅ’ Esta devoluciÃƒÂ³n ya fue procesada (estado: {estado})"
            
            # Actualizar estado de la devoluciÃƒÂ³n a COMPLETADA
            cursor.execute("""
                UPDATE Devoluciones 
            ...
            # Verificar si se devolviÃƒÂ³ todo lo entregado
            cursor.execute("""
                SELECT 
                    sm.CantidadEntregada,
                    ISNULL((SELECT SUM(d.CantidadDevuelta) FROM Devoluciones d 
            ...
            if row:
                ...
                # Si se devolviÃƒÂ³ todo, cambiar estado a "Devuelta"
            ...
            conn.commit()
            return True, f"Ã¢Å“â€¦ DevoluciÃƒÂ³n aprobada. Se reintegraron {cantidad_devuelta} unidades al inventario."
            
        except Exception as e:
            conn.rollback()
            print(f"Ã¢ÂÅ’ Error aprobando devoluciÃƒÂ³n: {e}")
            import traceback
            traceback.print_exc()
            return False, f"Ã¢ÂÅ’ Error: {str(e)}"
        finally:
    ...
    def rechazar_devolucion(devolucion_id, usuario_rechaza, observaciones=""):
        """
        Rechaza una solicitud de devoluciÃƒÂ³n.
        """
        conn = get_database_connection()
        if conn is None:
            return False, "Ã¢ÂÅ’ Error de conexiÃƒÂ³n a la base de datos"
        ...
        try:
            ...
            if row[0] != 'PENDIENTE':
                return False, f"Ã¢ÂÅ’ Esta devoluciÃƒÂ³n ya fue procesada (estado: {row[0]})"
            ...
            conn.commit()
            return True, "Ã¢Å“â€¦ DevoluciÃƒÂ³n rechazada"
            
        except Exception as e:
            conn.rollback()
            print(f"Ã¢ÂÅ’ Error rechazando devoluciÃƒÂ³n: {e}")
            return False, f"Ã¢ÂÅ’ Error: {str(e)}"
        finally:
            cursor.close()
            conn.close()
    ...
    def obtener_devolucion_pendiente(solicitud_id):
        """
        Obtiene la devoluciÃƒÂ³n pendiente de aprobaciÃƒÂ³n para una solicitud.
        """
        conn = get_database_connection()
        if conn is None:
            return None
        ...
        try:
            ...
            print(f"Ã¢ÂÅ’ Error obteniendo devoluciÃƒÂ³n pendiente: {e}")
            return None
        finally:
            cursor.close()
            conn.close()
    ...
    def tiene_devolucion_pendiente(solicitud_id):
        """Verifica si una solicitud tiene devoluciÃƒÂ³n pendiente de aprobaciÃƒÂ³n"""
        conn = get_database_connection()
        if conn is None:
            return False
