...
class InventarioCorporativoModel:
    ...
    @staticmethod
    def obtener_por_oficina_codigo(codigo_oficina):
        """
        Obtener productos asignados a una oficina especifica
        codigo_oficina: Codigo de la oficina (ej: 'COQ', 'BOG', etc)
        """
        conn = get_database_connection()
        if not conn:
            return []
        ...
        try:
            ...
            query = """
                SELECT 
                    ...
                    pr.NombreProveedor AS ProveedorNombre,
                    p.ValorUnitario,
                    p.CantidadDisponible AS Cantidad,
                    p.CantidadMinima,
                    p.Estado,
                ...
                LEFT JOIN Oficinas o ON a.OficinaId = o.OficinaId
                WHERE p.Activo = 1
                  AND p.UbicacionTipo = 'OFICINA'
                  AND o.CodigoOficina = ?
                ORDER BY p.NombreProducto
            """
            cursor.execute(query, (codigo_oficina,))
            cols = [c[0] for c in cursor.description]
            productos = [dict(zip(cols, r)) for r in cursor.fetchall()]
            return productos
        except Exception as e:
            print(f"Error obteniendo productos por oficina {codigo_oficina}: {e}")
            return []
        finally:
            if cursor:
                cursor.close()
    ...
    def solicitar_devolucion(producto_id, oficina_id, cantidad, motivo, usuario_solicita):
        ...
        try:
            ...
            print(f"Devolución solicitada: Producto {producto_id}, Oficina {oficina_id}, Usuario {usuario_solicita}")
    ...
    def aprobar_devolucion(devolucion_id, usuario_aprueba, observaciones=''):
        ...
        try:
            ...
            print(f"Devolución aprobada: ID {devolucion_id}, Producto {nombre_producto}, por {usuario_aprueba}")
            ...
            print(f"Error en aprobar_devolucion: {e}")
            if conn:
                conn.rollback()
    ...
    def rechazar_devolucion(devolucion_id, usuario_rechaza, motivo_rechazo):
        ...
        try:
            ...
            print(f"Devolución rechazada: ID {devolucion_id}, por {usuario_rechaza}")
            ...
            print(f"Error en rechazar_devolucion: {e}")
            if conn:
                conn.rollback()
    ...
    def obtener_devoluciones_pendientes():
        ...
        try:
            ...
            print(f"Error en obtener_devoluciones_pendientes: {e}")
            return []
        finally:
            if cursor:
                cursor.close()
    ...
    def obtener_devoluciones_por_oficina(oficina_id):
        ...
        try:
            ...
            print(f"Error en obtener_devoluciones_por_oficina: {e}")
            return []
        finally:
            if cursor:
                cursor.close()
    ...
    def dar_de_baja_producto(producto_id, asignacion_id, motivo, usuario_accion):
        ...
        try:
            ...
            print(f"Producto dado de baja: {codigo_producto} - {nombre_producto}, por {usuario_accion}")
            ...
            print(f"Error en dar_de_baja_producto: {e}")
            if conn:
                conn.rollback()
    ...
    def obtener_productos_devueltos():
        ...
        try:
            ...
            print(f"Error en obtener_productos_devueltos: {e}")
            return []
        finally:
            if cursor:
                cursor.close()
    ...
    def solicitar_traspaso(producto_id, oficina_origen_id, oficina_destino_id,
        ...
        try:
            ...
            print(f"Traspaso solicitado: Producto {producto_id}, {nombre_oficina_origen} -> {nombre_oficina_destino}, Usuario {usuario_solicita}")
            ...
            print(f"Error en solicitar_traspaso: {e}")
            if conn:
                conn.rollback()
    ...
    def aprobar_traspaso(traspaso_id, usuario_aprueba, observaciones=''):
        ...
        try:
            ...
            print(f"Traspaso aprobado: ID {traspaso_id}, Producto {nombre_producto}, por {usuario_aprueba}")
            ...
            print(f"Error en aprobar_traspaso: {e}")
            if conn:
                conn.rollback()
    ...
    def rechazar_traspaso(traspaso_id, usuario_rechaza, motivo_rechazo):
        ...
        try:
            ...
            print(f"Traspaso rechazada: ID {traspaso_id}, por {usuario_rechaza}")
            ...
            print(f"Error en rechazar_traspaso: {e}")
            if conn:
                conn.rollback()
    ...
    def obtener_traspasos_pendientes():
        ...
        try:
            ...
            print(f"Error en obtener_traspasos_pendientes: {e}")
            return []
        finally:
            if cursor:
                cursor.close()
    ...
    def obtener_traspasos_por_oficina(oficina_id):
        ...
        try:
            ...
            print(f"Error en obtener_traspasos_por_oficina: {e}")
            return []
        finally:
            if cursor:
                cursor.close()
